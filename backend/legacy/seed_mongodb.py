"""
Script pentru popularea bazei de date MongoDB Atlas cu date inițiale
Rulează: python seed_mongodb.py
"""
...
def seed_database():
    """Populează baza de date MongoDB"""

    # Conectare la MongoDB
    MONGODB_URI = os.getenv('MONGODB_URI')
    if not MONGODB_URI:
        print("❌ MONGODB_URI nu este setat în .env!")
        return
    ...
    print("\n" + "=" * 50)
    print("🌱 Populare bază de date BAC Prep AI")
    print("=" * 50)

    # 1. Adaugă exerciții
    print("\n📝 Adăugare exerciții...")
    exercises.delete_many({})  # Șterge exercițiile existente
    exercises.insert_many(EXERCISES_DATA)
    print(f"✅ {len(EXERCISES_DATA)} exerciții adăugate!")

    # 2. Creează utilizator demo
    print("\n👤 Creare utilizator demo...")
    users.delete_many({'username': 'demo'})  # Șterge dacă există
    demo_user = {
        'id': 1,
    ...
    print("✅ Utilizator demo creat! (username: demo)")

    # 3. Adaugă achievements
    print("\n🏆 Adăugare achievements...")
    achievements_data = [
    ...
    achievements.insert_many(achievements_data)
    print(f"✅ {len(achievements_data)} achievements adăugate!")

    # Statistici finale
    print("\n" + "=" * 50)
    print("📊 Statistici bază de date:")
    print(f"   • Exerciții: {exercises.count_documents({})}")
    print(f"   • Utilizatori: {users.count_documents({})}")
    print(f"   • Achievements: {achievements.count_documents({})}")
    print("=" * 50)
    print("\n🎉 Baza de date a fost populată cu succes!")
    print("\n💡 Acum poți rula: python app.py")
